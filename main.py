        ...
        if res.name() != None:
            if res.whether() == [False] * 3:
                ...
                if res.whether()[2] == True:
                    print("\t\tin the eveningg the weather will be suitable for skiing".format(res.name()))
                if res.whether()[2] == False:
                    print("\t\tin the evening the weather will not be suitable for skiing".format(res.name()))
            # print()
...
def get_all_data(country, date):
    ...
    if not ch or ch == None:
        # print("You entered incongruous date")
        return None
    if ch:
